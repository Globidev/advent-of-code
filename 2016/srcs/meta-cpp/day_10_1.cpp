...
// Making up an end condition similar to the one of the problem
// I cannot use the real one since using the real input takes too long
let end_condition = State::queue >> (at & 0_c) >> demux(and_)(
    Bot::low  >> equal.to(id_c<2>),
    Bot::high >> equal.to(just(id_c<3>))
);

let day_10_1_impl =
    (unpack & State::constructor) >>
    ((while_ | (not_ << end_condition)) & pop_bot) >>
    State::queue >> (at & 0_c) >> Bot::id;

let day_10_1 = variadicly(day_10_1_impl);
