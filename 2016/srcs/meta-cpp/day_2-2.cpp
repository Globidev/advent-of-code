...
enum Key { K1, K2, K3, K4, K5, K6, K7, K8, K9, KA, KB, KC, KD, K_ };
...
let position_to_key(Position pos) {
    return keypad[pos.y][pos.x];
}

let direction_vector(Direction d) {
    switch(d) {
        case U: return v_up;
...
let move_on_keypad(Position pos, Direction dir) {
    let_ v = direction_vector(dir);
    let_ nx = pos.x + v.dx,
         ny = pos.y + v.dy;
    let_ new_position = Position { nx, ny };
    ...
    if (!out_of_bounds && position_to_key(new_position) != K_)
        return new_position;
    else
        return pos;
}
...
let apply_instructions = [](auto position, auto instructions) {
    return fold(
        instructions,
        position,
        move_on_keypad
...
let day_2_2_impl =
    (scan_left & initial_position & apply_instructions) >>
    (remove_at & 0_c) >>
    (transform & position_to_key);

let day_2_2 = variadicly(day_2_2_impl);

int main() {
    // Example
    static_assert(day_2_2(
        make_tuple(U, L, L),
        make_tuple(R, R, D, D, D),
        make_tuple(L, U, R, D, L),
        make_tuple(U, U, U, U, D)
    ) == make_tuple(K5, KD, KB, K3));
